class Puzzle {
    ...
    * @param disperseY - range (-1, 1)
     */
    constructor(id, posx, posy, disperseX, disperseY, gameProp, pathRoot) {
        this.id = id;
        this.posx = posx;
        this.posy = posy;
        this.width = gameProp.CARD_WIDTH;
        this.height = gameProp.CARD_HEIGHT;
        ...
        this.enableRotation = true; // flag to enable/disable rotation of the card.
        this.shouldScaleWhenActive = true;

        this.easeInFactorInitialization = this.gameProp.POSY_INITIAL_DECK; // how far the card moves at the initialization.
        ...
        if (disperseX !== undefined && disperseY !== undefined){
            this.easeInFactorDisperse = map(disperseX, -1, 1, this.gameProp.EASE_IN_FACTOR_DISPERSE_LEFT_LIMIT,
                this.gameProp.EASE_IN_FACTOR_DISPERSE_RIGHT_LIMIT); // how far the card moves posX after disperse.
            this.posyOffsetDisperse = map(disperseY, -1, 1, this.gameProp.POSY_OFFSET_DISPERSE_LEFT_LIMIT,
                this.gameProp.POSY_OFFSET_DISPERSE_RIGHT_LIMIT); // how far the card moves posY after disperse
        }
        ...
        this.puzzleImg = loadImage(pathRoot + '/' + gameProp.gameState.puzzleImageId + '/' + id + '.png');
    }
    ...
    moveToStartingPoint() {
        if (!this.dispersed) {
            ...
            this.posy = this.initialPosY - easedValue * (this.initialPosY - this.gameProp.POSY_INITIAL_CARD);
    ...
    returnToScatteredPosition() {
        if (this.shouldBeReturned) {
            ...
            this.posy = this.gameProp.POSY_INITIAL_CARD + easedValue * (this.initialPosYAfterScatter - this.gameProp.POSY_INITIAL_CARD);
            // }
    ...
    draw() {
        ...
        image(this.puzzleImg, -this.puzzleImg.width/2, -this.puzzleImg.height/2);
        fill(0);
        textSize(44);
        // text(this.id, 0, 0);
        pop();
